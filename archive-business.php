<?php
get_header();

// ── Fetch all businesses for tag/location counts, render only the first page ──

$per_page       = WHITBYANCHOR_BUSINESSES_PER_PAGE;
$all_businesses = get_businesses( [ 'limit' => 1000 ] );

$total      = count( $all_businesses );
$first_page = array_slice( $all_businesses, 0, $per_page );
$has_more   = $total > $per_page;

// ── Build tag & location counts from the full set ─────────────────────────────

$tag_counts      = [];
$location_counts = [];

foreach ( $all_businesses as $business ) {
	$post_id = $business['post']->ID;

	$tags = get_the_terms( $post_id, 'business_tag' );
	if ( $tags && ! is_wp_error( $tags ) ) {
		foreach ( $tags as $term ) {
			if ( isset( $tag_counts[ $term->slug ] ) ) {
				$tag_counts[ $term->slug ]['count']++;
			} else {
				$tag_counts[ $term->slug ] = [ 'name' => $term->name, 'slug' => $term->slug, 'count' => 1 ];
			}
		}
	}

	$locations = get_the_terms( $post_id, 'business_location' );
	if ( $locations && ! is_wp_error( $locations ) ) {
		foreach ( $locations as $term ) {
			if ( isset( $location_counts[ $term->slug ] ) ) {
				$location_counts[ $term->slug ]['count']++;
			} else {
				$location_counts[ $term->slug ] = [ 'name' => $term->name, 'slug' => $term->slug, 'count' => 1 ];
			}
		}
	}
}

uasort( $tag_counts,      fn( $a, $b ) => strcasecmp( $a['name'], $b['name'] ) );
uasort( $location_counts, fn( $a, $b ) => strcasecmp( $a['name'], $b['name'] ) );
?>

<main id="primary" class="site-main business-directory">
	<div>
		<h1>Whitby Business Directory</h1>
		<p>Some blurb here.</p>

		<section class="businesses">
			<header>
				<h2>Filter Businesses</h2>

				<?php if ( $tag_counts ) : ?>
					<div>
						<label for="business-tag-select">
							<?php esc_html_e( 'Business type', 'whitbyanchor' ); ?>
						</label>
						<select id="business-tag-select" name="business_tag">
							<option value=""><?php esc_html_e( 'Everything', 'whitbyanchor' ); ?></option>
							<?php foreach ( $tag_counts as $tag ) : ?>
								<option value="<?php echo esc_attr( $tag['slug'] ); ?>">
									<?php echo esc_html( $tag['name'] ); ?> (<?php echo intval( $tag['count'] ); ?>)
								</option>
							<?php endforeach; ?>
						</select>
					</div>
				<?php endif; ?>

				<?php if ( $location_counts ) : ?>
					<div>
						<label for="business-location-select">
							<?php esc_html_e( 'Location', 'whitbyanchor' ); ?>
						</label>
						<select id="business-location-select" name="business_location">
							<option value=""><?php esc_html_e( 'Anywhere', 'whitbyanchor' ); ?></option>
							<?php foreach ( $location_counts as $loc ) : ?>
								<option value="<?php echo esc_attr( $loc['slug'] ); ?>">
									<?php echo esc_html( $loc['name'] ); ?> (<?php echo intval( $loc['count'] ); ?>)
								</option>
							<?php endforeach; ?>
						</select>
					</div>
				<?php endif; ?>
			</header>

			<div class="search">
				<h2>Search businesses</h2>
				<input type="text" id="business-search" />
			</div>

			<?php if ( $all_businesses ) : ?>

				<div id="businesses-list">
					<?php foreach ( $first_page as $index => $business ) : ?>
						<?php echo whitbyanchor_render_business_article( $business ); ?>
						<?php if ( $index === 3 ) : ?>
							<div class="ad-wide">
								<?php echo do_shortcode( '[newspaper_advert placement="category_top" category_id="221"]' ); ?>
							</div>
						<?php endif; ?>
					<?php endforeach; ?>
				</div>

				<?php if ( $has_more ) : ?>
					<button id="businesses-load-more" type="button"
							data-page="1"
							data-per-page="<?php echo esc_attr( $per_page ); ?>">
						<?php esc_html_e( 'Load more businesses', 'whitbyanchor' ); ?>
					</button>
				<?php endif; ?>

			<?php else : ?>
				<p><?php esc_html_e( 'No businesses found — try changing your filters.', 'whitbyanchor' ); ?></p>
			<?php endif; ?>

		</section>
	</div>

	<section>
		<?php echo do_shortcode( '[newspaper_advert placement="category_top" category_id="221"]' ); ?>

		<?php
		echo '<h2 class="category-heading">Business News</h2>';

		$news_query = new WP_Query( [
			'category_name'  => 'businesses',
			'posts_per_page' => 4,
			'orderby'        => 'date',
			'order'          => 'DESC',
		] );

		if ( $news_query->have_posts() ) :
			while ( $news_query->have_posts() ) :
				$news_query->the_post();
				echo '<article class="flow">';
				echo '<figure>';
				the_post_thumbnail( 'full' );
				echo '</figure>';
				echo '<figcaption>';
				the_post_thumbnail_caption();
				echo '</figcaption>';
				the_title( '<h2>', '</h2>' );
				echo '<div class="entry-meta">';
				echo '<img src="' . get_stylesheet_directory_uri() . '/icons/apple-icon-180x180.png" alt="Whitby Anchor" />';
				whitbyanchor_posted_on();
				whitbyanchor_posted_by();
				echo '</div>';
				echo '<p class="excerpt">' . get_the_excerpt() . '</p>';
				echo '<a class="article-link" href="' . get_the_permalink() . '"><span>Read: ' . get_the_title() . '</a>';
				echo '</article>';
			endwhile;
			wp_reset_postdata();
		endif;
		?>

		<?php echo do_shortcode( '[newspaper_advert placement="category_bottom" category_id="221"]' ); ?>
	</section>
</main>

<script>
const businessesConfig = <?php echo wp_json_encode( $businesses_config ); ?>;
</script>

<?php
get_sidebar();
get_footer();
